...
const feed = mongoose.model('feedback');
...
router.post('/edit_product',(req,res) => {
    console.log("Reached with " + req.body._id + "and new data is " + req.body);
    updateRecord(req,res);
});
...
function updateRecord(req,res){
    product.findOneAndUpdate({_id:req.body._id},{ new: true },req.body,(err,doc)=>{
        if(!err){
            console.log("reached in update with " + req.body._id + " and " + req.body);
            res.redirect('/Index/manage_product');
        }
        else{
...
router.get('/edit_product/:id',(req, res)=>{
    product.findById(req.params.id,(err,doc) => {
        if(!err){
            console.log("Reached in edit_product with " + req.params.id),
            res.render("Index/edit_product",{
                viewTitle:"Update Product",
                Product: doc
            });
